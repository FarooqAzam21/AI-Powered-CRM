...
def broadcast_deal_update(self, deal_id: int, user_id: int, workspace_id: Optional[int] = None):
    """Broadcast deal update to connected clients"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting deal update: deal_id={deal_id}, workspace_id={workspace_id}")
...
def broadcast_deal_closed(self, deal_id: int, user_id: int, workspace_id: Optional[int] = None):
    """Broadcast deal closed event"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting deal closed: deal_id={deal_id}, workspace_id={workspace_id}")
...
def broadcast_territory_alert(self, territory_name: str, user_id: int, workspace_id: Optional[int] = None):
    """Broadcast territory alerts"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting territory alert: territory={territory_name}, workspace_id={workspace_id}")
        
        t_query = db.query(TerritoryMetrics).filter(
            TerritoryMetrics.territory_name == territory_name,
            TerritoryMetrics.user_id == user_id
        )
        if workspace_id is not None:
            t_query = t_query.filter(TerritoryMetrics.workspace_id == workspace_id)
        territory = t_query.first()
...
def broadcast_forecast_alert(self, user_id: int, workspace_id: Optional[int] = None):
    """Broadcast forecast status alert"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting forecast alert: user_id={user_id}, workspace_id={workspace_id}")
        
        alert = DashboardService.generate_forecast_alert(db, user_id, workspace_id=workspace_id)
...
def broadcast_activity_event(self, activity_id: int, workspace_id: Optional[int] = None):
    """Broadcast activity created event"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting activity event: activity_id={activity_id}, workspace_id={workspace_id}")
...
def broadcast_recommendation_event(self, recommendation_id: int, workspace_id: Optional[int] = None):
    """Broadcast recommendation generated event"""
    db = SessionLocal()
    try:
        logger.info(f"📡 [Dashboard] Broadcasting recommendation event: rec_id={recommendation_id}, workspace_id={workspace_id}")
    ...
    try:
        ...
        for user_id in connected_user_ids:
            try:
                ...
                # Get current metrics
                metrics = DashboardService.get_dashboard_metrics(db, user_id, workspace_id=ws_id)
    ...
    try:
        ...
        for user_id in connected_user_ids:
            try:
                ...
                snapshot = DashboardService.get_pipeline_snapshot(db, user_id, workspace_id=ws_id)
    ...
    try:
        ...
        for user_id in connected_user_ids:
            try:
                ...
                snapshot = DashboardService.get_territory_snapshot(db, user_id, workspace_id=ws_id)
